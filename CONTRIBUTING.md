...
configure beman.indices_view to fetch them automatically via CMake FetchContent.
...
```bash
cmake -LH -S . -B build | grep "BEMAN_INDICES_VIEW" -C 2
```
...
This is required so that users of `beman.indices_view` can use
`find_package(beman.indices_view)` to locate the library.
